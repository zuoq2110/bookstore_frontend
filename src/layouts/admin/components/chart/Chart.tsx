...
const Chart: React.FC<ChartProps> = (props) => {
	...
	function handleChangeOrderByStatistics(event: SelectChangeEvent<string>) {
	...
	function handleChangeOrderByLatestDate(event: SelectChangeEvent<string>) {
	...
	function handleChangeOrderByMonthlyOfYear(event: SelectChangeEvent<string>) {
	...
	const updateData = useMemo(
		() => (option: string, year?: string, latestDays?: string[]) => {
			switch (option) {
				case "yearly":
					const newDataNumberOfOrder_Yearly = new Array(BY_YEAR.length).fill(0);
					const newDataTotalPrice_Yearly = new Array(BY_YEAR.length).fill(0);
					props.orders?.forEach((order) => {
						const orderDate = new Date(order.ngayTao);
						if (order.tinhTrangDonHang === "Thành công") {
							const year = orderDate.getFullYear() + "";
							newDataNumberOfOrder_Yearly[BY_YEAR.indexOf(year)] += 1;
							newDataTotalPrice_Yearly[BY_YEAR.indexOf(year)] += order.tongTien
						}
					})
				...
				case "daily":
					...
					props.orders?.forEach((order) => {
						const orderDate = new Date(order.ngayTao)
						const orderDateFormated = formatDate(orderDate)
						if (order.tinhTrangDonHang === "Thành công" &&
							latestDays?.includes(orderDateFormated)
						) {
							newDataNumberOfOrder_Daily[latestDays.indexOf(orderDateFormated)] += 1
							newDataTotalPrice_Daily[latestDays.indexOf(orderDateFormated)] += order.tongTien
						}
					})
	...
	}, [orderByStatistics, orderByMonthlyOfYear, updateData])
	...
	useEffect(() => {
		...
		props.orders?.forEach((order) => {
			...
			} else if (order.tinhTrangDonHang === "Bị hủy") {
				orderFailureTotal++;
			} else if (order.tinhTrangDonHang === "Đang giao hàng") {
